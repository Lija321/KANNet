...
def get_dataloaders(
    ...
    root = resolve_data_root(data_root, "food101")
    train_full = Food101(root=root, split="train", transform=preprocess_train, download=True)
    val_full = Food101(root=root, split="train", transform=preprocess_eval, download=True)
    test_dataset = Food101(root=root, split="test", transform=preprocess_eval, download=True)
