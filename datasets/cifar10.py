...
def get_dataloaders(
    ...
    root = resolve_data_root(data_root, "cifar10")
    train_full = CIFAR10(root=root, train=True, transform=preprocess_train, download=True)
    val_full = CIFAR10(root=root, train=True, transform=preprocess_eval, download=True)
    test_dataset = CIFAR10(root=root, train=False, transform=preprocess_eval, download=True)
